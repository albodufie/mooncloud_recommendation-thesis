...
la prima volta nel 2000 all'interno di una tesi per il dottorato di Roy Fielding. Questo approccio architetturale venne ideato per creare 
...
Il secondo approccio analizzato è il \textit{Nested Set Model}, il quale è stato utilizzato per l'implementazione delle tassonomie delle le Evaluation e 
dei Controlli (politiche per verificare il soddisfacimento dei requisiti di sicurezza di un Target o asset indicato dall'utente) all'interno del 
progetto.\hfill\break
...
\begin{figure}[ht!]
    \centering
    \includegraphics[scale=0.65]{images/MC_Rec_NSM_Container(R).png}
    \caption{Esempio di gestione di dati in modo gerarchico secondo il Nested Set Model, utilizzando una parte di quelli presenti nel database del 
...
\begin{figure}[ht!]
    ...
    \caption{Esempio della gestione di dati in modo gerarchico secondo il Nested Set Model, utilizzando una parte di quelli presenti nel database del 
    progetto in questione.}
