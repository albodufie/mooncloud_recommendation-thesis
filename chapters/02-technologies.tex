...
nel capitolo successivo vengono approfonditi a livello pratico i sistemi di raccomandazione basati su tecniche di Filtraggio
Collaborativo Memory-based i quali sono stati utilizzati per l'implementazione della soluzione.
%
...
Django supporta lo sviluppo di applicazioni con le seguenti caratteristiche \cite{django-documentation}.
\begin{description}
    ...
    fa uso del principio "Don't Repeat Yourself" (DRY) così da ridurre al minimo le duplicazioni non necessarie, diminuendo la quantità di codice.
    ...
    sia Linux, Windows o Mac OS. Per di più, è ben supportato da molti web hosting provider, che spesso provvedono a specifche infrastrutture
...
\begin{figure}[ht!]
    \centering
    \includegraphics[scale=0.5]{Django_doc}
...
\lstset{style=python_code_style}
\begin{lstlisting}[language=Python, label=lst:URLex1, caption={\ }]
...
restituisce un risultato, in questo caso viene richiamata la View che implementa il sistema di raccomandazione Item-based, la quale effettua una ricerca 
...
\lstset{style=python_code_style}
\begin{lstlisting}[language=Python, label=lst:URLex2, caption={\ }]
...
\begin{figure}[ht!]
    \centering
    \includegraphics[scale=0.6]{api_rest}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[scale=0.55]{Docker_Config_Container}
...
Docker, considera i Container come macchine virtuali modulari estremamente leggere, offrendo la flessibilità di crearli, distribuirli, 
copiarli e spostarli da un ambiente all'altro, ottimizzando così le app per il cloud.\hfill\break
Essi forniscono una modalità standard per impacchettare il codice delle applicazioni, le configurazioni e le dipendenze, in un singolo oggetto e 
...
%
Questo sistema crea un livello di astrazione fra i Container e il sistema operativo ospitante e gestisce la loro attivazione e disattivazione. 
...
PostgreSQL, un sistema di gestione di database relazionali ad oggetti. In generale le tabelle contenute in questo tipo di base di dati non permettono la 
...
Durante lo studio compiuto per la realizzazione di questa soluzione sono stati analizzati diversi approcci per poter gestire le informazioni in modo gerarchico, 
...
\begin{figure}[ht!]
    \centering
    \includegraphics[scale=0.58]{MC_Rec_Tree}
...
\newpage
%
\subsection{Adjacency List Model}
...
Per ogni nodo, una funzione di questo tipo crea una nuova istanza di se stessa e ogni istanza occupa una porzione di memoria e impiega un certo 
tempo per inizializzarsi, più grande è l'albero e più questo processo sarà portato a termine in maggior tempo.
%
\subsection{Nested Set Model}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[scale=0.73]{MC_Rec_NSM_Container(R)}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[scale=0.57]{MC_Rec_NSM_Tree}
...
può notare che questa query funziona in modo indipendente dalla profondità della tassonomia. Inoltre, non è necessario preoccuparsi del valore \texttt{rght} 
...
\begin{figure}
    \centering
    \includegraphics[scale=0.58]{MC_Rec_NSM_Container}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[scale=0.7]{recommender_systems}
...
Inoltre, nel caso siano presenti molti contenuti e proprietà associati agli item, allora vengono consumate molte 
risorse e tempo per poter analizzarli. Per quanto riguarda il CF, una raccomandazione 
...
\begin{description}
    ...
    \item[Memorizzazione di dati:] la quantità di dati definisce quanto efficace un modello di raccomandazione può di 
...
Questi sistemi tentano di predire la preferenza che un utente darebbe a un item basandosi sulle preferenze date da altri 
