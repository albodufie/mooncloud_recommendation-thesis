...
\end{description}
In generale, un tradizionale sito web resta in attesa di richieste HTTP da parte del browser web (o altri client) degli utenti; nel momento in cui riceve 
...
costituiti da relazioni padre-figlio che non possono essere rappresentate in modo naturale nelle tabelle di questo tipo.
...
organizzazione di un business, categorie per gestire contenuti e di prodotti.
%
\begin{figure}[ht!]
    \centering
...
\end{figure}
\hfill\break
Ci sono differenti modelli per poter gestire le informazioni in modo gerarchico, i più importanti che sono stati
presi in considerazione sono i seguenti.
%
\subsection{The Adjacency List Model}
Il primo approccio, e quello più semplice da implementare, è chiamato \textit{Adjacency List Model} o metodo ricorsivo; 
viene definito tale perché il suo funzionamento si basa su una funzione che itera per tutto l'albero.\hfill\break
In questo modello, ogni item (nodo dell'albero), contenuto nella tabella, ha associato un puntatore al suo item padre, in particolare 
il nodo radice ha un puntatore a un valore NULL per l'item padre, visto che è il nodo di partenza.\hfill\break
...
in un database. L'\textit{item} è una qualsiasi cosa di interesse agli utenti, come prodotti, libri o giornali. Quando si esegue 
...
necessario trattarli in contemporanea perché aumentare solamente la scalabilità diminuirebbe la sua qualità e viceversa 
