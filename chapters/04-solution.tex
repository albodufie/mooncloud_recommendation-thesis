...
% visto che il codice l'ho messo nel capitolo precendente, posso mettere qui degli esempi e mostrare le url per richiamare
...
In generale gli algortmi di raccomandazione implementati in questo progetto, e descritti nel Capitolo precendente, vengono richiamati e 
utilizzati come API REST, un tipo di architettura basata sul protocollo HTTP. Un sistema REST, per funzionare, prevede una struttura degli 
URL ben definita (atta a identificare univocamente una risorsa o un insieme di risorse) l'utilizzo dei verbi HTTP specifici per il recupero 
di informazioni (GET), per la modifica (POST, PUT, PATCH, DELETE) e per altri scopi (OPTIONS, ecc.).\hfill\break
In questo caso ad ogni URL è associata una azione particolare la quale va a richiamare la View che permette di determinare un insieme di 
...
\end{lstlisting}
Sulla base dell'URL che viene richiamato e sul parametro (come \textit{user\_other\_id}, \textit{item\_other\_id}, ecc.) verranno fornite le 
...
e restiutiscono una risposta HTTP in formato JSON, queste risposte contengono una lista di dizionari e ciascuno contiene l'other\_id della Evaluation 
...
# TARGET RECOMMENDATION ALGORITHM & API REST

# Target recommendation algorithm
def target_recommendation_alg(target_type_id):
    """
    ...
    """
    # Retrieving all the evaluations in the database
    evaluations = Evaluation.objects.filter(node_type="eva")

    # Saving in the target_evaluations list the evaluations which controls have target_type_id equal to target_type_id
    target_evaluations = []
    for evaluation in evaluations:  # Scanning all the evaluations
        for evaluation_controls in evaluation.controls.filter(target_type_id=target_type_id):
    ...
    target_evaluations_serializer = EvaluationSerializerRecommendation(target_evaluations, many=True)

    return target_evaluations_serializer.data
\end{lstlisting}
\lstset{style=python_code_style}
\begin{lstlisting}[language=Python, label=lst:view_rec_ex, caption={Esempi di chiamata e risposta HTTP per i diversi algoritmi di raccomandazione.}]
